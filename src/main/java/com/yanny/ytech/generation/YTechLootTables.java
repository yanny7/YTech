...
class YTechLootTables extends LootTableProvider {
    ...
    private static class YTechBlockLootSub extends BlockLootSubProvider {
        ...
        protected void generate() {
            ...
            Registration.REGISTRATION_HOLDER.kineticNetwork().forEach((type, materialMap) -> materialMap.forEach((material, holder) -> dropSelf(holder.block().get())));
        }
        ...
        protected Iterable<Block> getKnownBlocks() {
            Stream<Block> stream = Stream.of(
                    ...
                    Registration.REGISTRATION_HOLDER.kineticNetwork().values().stream().flatMap((map) -> map.values().stream().flatMap((holder) -> holder.block().stream()))
            ).flatMap(i -> i);
