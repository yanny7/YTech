...
class YTechBlockTagsProvider extends BlockTagsProvider {
    ...
    private static Block[] filteredMaterials(YTechBlocks.MaterialBlock block, EnumSet<MaterialType> exclude) {
        return block.entries().stream().filter((entry) -> !exclude.contains(entry.getKey())).sorted(Comparator.comparing(t -> t.getKey().key)).map(Map.Entry::getValue).map(RegistryObject::get).toArray(Block[]::new);
    }

    private static Block[] sortedMaterials(YTechBlocks.MaterialBlock block) {
        return block.entries().stream().sorted(Comparator.comparing(t -> t.getKey().key)).map(Map.Entry::getValue).map(RegistryObject::get).toArray(Block[]::new);
    }
}
