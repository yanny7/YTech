...
@OnlyIn(Dist.CLIENT)
public class DeerModel<T extends Animal> extends AgeableListModel<T> {
    ...
    public DeerModel(@NotNull ModelPart root) {
        super(true, 20.0F, 0.0F, 2.5F, 2.0F, 24);
    ...
    protected Iterable<ModelPart> bodyParts() {
        return ImmutableList.of(this.body, this.fl_foot, bl_foot, fr_foot, br_foot);
    }
    ...
    public void setupAnim(@NotNull T entity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        ...
        if (entity instanceof DeerEntity deerEntity) {
            this.head.getChild("antlers").visible = deerEntity.hasAntler();
        }
    }
