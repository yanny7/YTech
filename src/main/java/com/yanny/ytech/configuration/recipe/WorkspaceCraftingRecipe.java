...
public record WorkspaceCraftingRecipe(PatternHolder patternHolder, ItemStack result) implements Recipe<Container> {
    ...
    public static class Serializer implements RecipeSerializer<WorkspaceCraftingRecipe> {
        private static final Codec<WorkspaceCraftingRecipe> CODEC = RecordCodecBuilder.create((instance) ->
                instance.group(
        ...
        @NotNull
        @Override
        public Codec<WorkspaceCraftingRecipe> codec() {
            return CODEC;
        }

        @NotNull
        @Override
        public WorkspaceCraftingRecipe fromNetwork(@NotNull FriendlyByteBuf buffer) {
            PatternHolder patternHolder = PatternHolder.fromNetwork(buffer);
            ItemStack result = buffer.readItem();
            return new WorkspaceCraftingRecipe(patternHolder, result);
        }

        @Override
        public void toNetwork(@NotNull FriendlyByteBuf buffer, WorkspaceCraftingRecipe recipe) {
            recipe.patternHolder.toNetwork(buffer);
    ...
    private record PatternHolder(NonNullList<Ingredient> ingredients, Optional<Pattern> data) {
        ...
        public static PatternHolder fromNetwork(FriendlyByteBuf buffer) {
            NonNullList<Ingredient> ingredients = NonNullList.withSize(27, Ingredient.EMPTY);
            ingredients.replaceAll(p_312742_ -> Ingredient.fromNetwork(buffer));
            return new PatternHolder(ingredients, Optional.empty());
        }
    }
