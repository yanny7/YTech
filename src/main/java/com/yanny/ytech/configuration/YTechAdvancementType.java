...
public enum YTechAdvancementType {
    ...
    IRRIGATION_SYSTEM(new Builder(YTechAdvancementType.STONE_AGE_GROUP, "irrigation_system").parent(() -> FIRST_STEPS.advancement)
            .display(YTechBlocks.AQUEDUCTS.of(MaterialType.TERRACOTTA).get(), AdvancementType.TASK, "Irrigation System", "Craft aqueduct for long distance water transportation.")
    ...
    public void generate(@NotNull HolderLookup.Provider registries, @NotNull Consumer<AdvancementHolder> saver, @NotNull ExistingFileHelper existingFileHelper) {
        ...
        builder.criterionApplicator.addCriterion(registries);
    ...
    private static class Builder {
        ...
        private ICriterionApplicator criterionApplicator;
        ...
        Builder hasOneOfItems(@NotNull ItemLike ...items) {
            criterionApplicator = (registries) -> {
                builder.addCriterion(getHasName(), InventoryChangeTrigger.TriggerInstance.hasItems(ItemPredicate.Builder.item().of(
                        registries.lookupOrThrow(Registries.ITEM), items).build()));
            };
            return this;
        }
        ...
        Builder hasAllItems(@NotNull ItemLike ...items) {
            criterionApplicator = (registries) -> {
                ...
                for (ItemLike item : items) {
                    builder.addCriterion(RecipeProvider.getHasName(item), InventoryChangeTrigger.TriggerInstance.hasItems(
                            ItemPredicate.Builder.item().of(registries.lookupOrThrow(Registries.ITEM), item).build()));
                }
            };
        ...
        final Builder hasOneOfTags(@NotNull TagKey<Item> ...tags) {
            criterionApplicator = (registries) -> {
                ...
                for (TagKey<Item> tag : tags) {
                    builder.addCriterion(getHasName(), InventoryChangeTrigger.TriggerInstance.hasItems(ItemPredicate.Builder.item().of(
                            registries.lookupOrThrow(Registries.ITEM), tag).build()));
                }
            };
        ...
        final Builder hasAllTags(@NotNull TagKey<Item> ...tags) {
            criterionApplicator = (registries) -> {
                ...
                for (TagKey<Item> tag : tags) {
                    builder.addCriterion(getHasName() + "_" + tag.location().getPath(), InventoryChangeTrigger.TriggerInstance.hasItems(
                            ItemPredicate.Builder.item().of(registries.lookupOrThrow(Registries.ITEM), tag).build()));
                }
            };
    ...
    public interface ICriterionApplicator {
        void addCriterion(@NotNull HolderLookup.Provider registries);
    }
}
